...
: QWidget(parent)
{
	...
	_rect_drawing        = false;
	_polygon_drawing     = false;
	_polygon_end_drawing = false;
	_lineThickness       = 3;
...
//	FinishJob();
	...
	{	
		...
		//IplImage* newImage = cvLoadImage( fileName.toStdString().c_str(), 1 );
...
void ImageViewer::saveLabelledResult(QString wholefilename)
{
    if(!_srcOcvImage) return;
    if(!_displayImage) return;
    if(!_result_labelImage) return;
    ...
    if(fDlg->exec() == QDialog::Accepted)
    {
        ...
        _result_labelImage->save(filename);
        _result_labelImage->save("F:/abrasivemarksystem/abrasivepic/"+wholefilename);
...
void ImageViewer::saveMask()
{
    if(!_mask.empty())
    {
        ...
        {
            QString filename = fDlg->selectedFiles()[0];
            std::string filename2 = filename.toLocal8Bit().constData();
            if(m_method == 0)
            {
                saveLabelMap(filename);
            }
            else
            {
                _mask = _mask * 255;
...
void ImageViewer::paintEvent(QPaintEvent * /* event */)
{
    ...
    else if(_labelType == 1 && _rect_drawing && bPaintable)
	{
	...
	else if(_labelType == 2 && _polygon_drawing && bPaintable)
	{
		// draw point with red color
		painter.setPen(QPen(Qt::red, 7));

		QPoint startPt = _polygonPointList.first();
		painter.drawPoint(startPt);
		...
		foreach(QPoint curPt, _polygonPointList)
		{
			...
			painter.drawLine(startPt, curPt);
			startPt = curPt;
		}
		if(_polygon_end_drawing) // end drawing
		{
			painter.setPen(QPen(_lineColor, _lineThickness, Qt::SolidLine));
			painter.drawLine(startPt, _polygonPointList.first());
			_polygon_drawing = false;
...
void ImageViewer::mousePressEvent(QMouseEvent *event)
{
	if ( bPaintable == true)
	{
		if (event->button() == Qt::LeftButton)
		{
			...
			else if(_labelType == 1) //plot rect
			{
			...
			else if(_labelType == 2)
			{
				...
				_polygon_drawing = true;

				_polygonPointList.push_back(lastPos);
				_polygonPointList_cv.push_back(cv::Point(lastPos.x(), lastPos.y())); // maybe fixed in the future;
...
void ImageViewer::mouseMoveEvent(QMouseEvent *event)
{
	if ( bPaintable == true )
	{
		if ((event->buttons() & Qt::LeftButton) && lastPos != QPoint(-1, -1) ) 
		{
			...
			else if(_labelType == 1 && _rect_drawing) // rect
			{
				_end_rect = event->pos();
				update();
			}
			else if(_labelType == 2 && _polygon_drawing) // polygon
			{
				// do nothing
			}
			lastPos = event->pos();
...
void ImageViewer::mouseReleaseEvent(QMouseEvent *event)
{
	if ( bPaintable == true )
	{
		if (event->button() == Qt::LeftButton && lastPos != QPoint(-1, -1)) 
		{
			...
			else if(_labelType == 1 && _rect_drawing)
			{
				_rect_drawing = false;
			...
			else if(_labelType == 2 && _polygon_drawing)
			{
				// do nothing
			}
...
// Mouse Double Click Event: finish polygon drawing
void ImageViewer::mouseDoubleClickEvent( QMouseEvent *event )
{
	if(_polygon_drawing)
	{
		_polygon_end_drawing = true;
        //update();
	}
}
...
{
	if(_polygon_end_drawing == false) return;
