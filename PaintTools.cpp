...
QStringList PaintTools::brushes() const
{
	return QStringList() << tr("Pencil") << tr("Air Brush") << tr("Random Letters");
}

QRect PaintTools::mousePress( const QString &brush, QPainter &painter, const QPoint &pos )
{
	return mouseMove(brush, painter, pos, pos);
}

QRect PaintTools::mouseMove( const QString &brush, QPainter &painter, const QPoint &oldPos, const QPoint &newPos )
{
	painter.save();
	...
	if ( brush == tr("Pencil") || brush == tr("pencil") ) 
	{
...
QRect PaintTools::mouseRelease(const QString & /* brush */,
									 QPainter & /* painter */,
									 const QPoint & /* pos */)
{
